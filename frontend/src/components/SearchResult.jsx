...
import { clearQueue, addToQueue, playNextSong, setCurrentSong } from '../slices/songPlayerSlice';
...
const SearchResult = () => {
    ...
    const handleFloatingPlayButtonClick = async () => {
        ...
        for (let index = 0; index < searchResult.songs.results.length; index++) {
            const response = await getSongData({ songId: searchResult.songs.results[index].id }).unwrap();
                            ...
                            <div className="flex">
                                {
                                    searchResult.topQuery.results.map((item, idx) => (
                                        <div onClick={()=>handleTopQueryClick(item)} onMouseEnter={() => setFloatingButton(true)} onMouseLeave={() => setFloatingButton(false)} key={idx} className={`flex flex-col max-w-md mt-4 shadow-lg duration-300 hover:shadow-sm w-[40vw] relative bg-amber-400`}>
                                            <h1 className='text-2xl'>Top Query</h1>
                                            <img src={item.image[2].link} loading="lazy" className="w-[max-content] h-48 rounded-full m-8" />
                                            ...
                                            {
                                                floatingButton ? (
                                                    <FaCirclePlay onClick={handleFloatingPlayButtonClick} className="text-7xl text-green-700 absolute bottom-0 right-0 mb-4 mr-4 z-50 hover:scale-110 duration-300 cursor-pointer" />
                                                ) : (
                                                    <></>
                                                )
                                            }
